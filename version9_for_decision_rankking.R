...
# For demonstration, printing the NA counts before replacement.
# You can remove these print statements in your final app.
message("NA counts in risk_attitude_df before replacement:")
print(colSums(is.na(risk_attitude_df)))

# Define the numeric columns where NA should be replaced by 0
numeric_score_columns <- c(
  "intuition_vs_data", "advisor_balance", "production_vs_profit",
  "comfort_with_process", "risk_attitude"
)

# Replace NA with 0 only in the specified numeric columns
risk_attitude_df <- risk_attitude_df %>%
  mutate(across(all_of(numeric_score_columns), ~replace_na(., 0)))

# Verify the replacement by printing NA counts again
message("NA counts in risk_attitude_df after replacement:")
print(colSums(is.na(risk_attitude_df)))
    ...
    if (input$variable_selector == "Review Process") {
      ...
      if (input$compare_mode == "compare_two") {
        ...
        if (input$variable_selector == "Decision Ranking") {
          ...
          # MODIFICATION START: Added logic to sort the decision ranking plot
          summary_ranking <- data %>%
            count(decision_name, difficulty_level) %>%
            group_by(decision_name) %>%
            mutate(percentage = n / sum(n)) %>%
            ungroup() %>%
            # Create a sorting metric based on the percentage of 'Very Difficult'
            group_by(decision_name) %>%
            mutate(sort_metric = sum(percentage[difficulty_level == "Very Difficult"])) %>%
            ungroup() %>%
            # Reorder the factor based on the metric
