...
ui <- fluidPage(
  ...
  fluidRow(
    column(12,
           # This single UI output now handles all summary displays
...
server <- function(input, output, session) {
  ...
  # The filtering function now includes logic for the role filter
  ...
  # Reactive for generating dynamic population names
  ...
  # Filter headers now use dynamic population names
  ...
  # Helper function to generate summary stats for a population
  ...
  # Helper function to create the UI for summary boxes
  ...
  output$main_plot <- renderPlot({
    # Get dynamic names for use in plots
