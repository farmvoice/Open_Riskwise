...
ui <- fluidPage(
  ...
  fluidRow(
    ...
    # View Mode (Now conditional)
    column(3,
           wellPanel(
             conditionalPanel(
               condition = "input.variable_selector != 'Decision Ranking'", # Condition added here
    ...
    column(3,
           wellPanel(
             h4("Population 1 / Peers Filters", align="center"),
    ...
    column(3,
           ...
           conditionalPanel(
             ...
             conditionalPanel(
               ...
               h4("Population 2 Filters", align = "center"),
               wellPanel(
...
server <- function(input, output, session) {
  ...
  # --- New: Set View Mode to 'single' and hide it when 'Decision Ranking' is selected ---
  observeEvent(input$variable_selector, {
    if (input$variable_selector == "Decision Ranking") {
      updateRadioButtons(session, "compare_mode", selected = "single")
      # The conditionalPanel in UI handles hiding/showing.
    }
  }, ignoreInit = TRUE) # ignoreInit = TRUE prevents it from running on app start-up
